...
const presetCallers = [
  { name: "Mom", number: "+1234567890", avatar: "👩‍💼" },
  { name: "Jake", number: "+1987654321", avatar: "👨‍💻" },
  { name: "Boss", number: "+1555123456", avatar: "👔" },
]
...
export default function FakeCallGenerator() {
  ...
  const [currentCaller, setCurrentCaller] = useState<(typeof presetCallers)[0] | null>(null)
  ...
  const handleCallAction = (action: "answer" | "decline" | "end") => {
    if (action === "answer") {
  ...
  return (
    <>
          ...
          <Card className="shadow-2xl border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm transition-all duration-300 hover:shadow-3xl">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3 justify-center">
                  {presetCallers.map((caller) => (
                    ...
                    >
                      <span className="mr-2">{caller.avatar}</span>
                      {caller.name}
                    </Button>
                  ))}
                </div>
              ...
              <Button
                ...
                disabled={isLoading || !selectedCaller || callStatus !== "idle"}
      ...
      {callStatus !== "idle" && currentCaller && (
        <CallInterface caller={currentCaller} status={callStatus} onAction={handleCallAction} />
      )}
    </>
  )
}
